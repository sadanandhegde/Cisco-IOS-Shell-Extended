...
class CiscoIOSHandlerExtended(CiscoHandlerBase):
    ...
    def add_vlan(self, vlan_range, port_list, port_mode, additional_info):
        """Adds save port config, configure speed and MTU
        :param vlan_range:
        :param port_list:
        :param port_mode:
    ...
    def get_link_attr_val(self, port, attr_name):
        """ Get value of an attribute on ta port
        ...
        # check if the port is connected anywhere
        try:
        ...
        # Loop thru all connectors in reservation, and then get value from the one that matches dut_port
        for conn in self.cloud_shell_api.GetReservationDetails(
                self.reservation_dict['ReservationId']).ReservationDescription.Connectors:
    ...
    def configure_interface_speed(self, port_list):
        ...
        for port in port_list.split('|'):
            ...
            # If it is a port-channel, correct the name
    ...
    def configure_interface_mtu(self, port_list):
        ...
        for port in port_list.split('|'):
            ...
            # If its a port-channel, correct the name
            if 'channel' in port_name.lower():
                port_name = port_name.replace('/', '-')
    ...
    def create_portchannel(self, port_list, port_channel_id='0'):
        ...
        Currently using only port-channel='0'
        :param port_channel_id: Find a port-channel if port-channel-id == 0
        :param port_list: list of ports separated by ";"
        :return:
        """
        ...
        for port in port_list.split(';'):
            ...
            vlan_id = re.search('switchport.*vlan.*\d+', self._send_command(
                'show running-config interface ' + port.split('/')[-1].replace('-', '/') + ' | include vlan'))
            if vlan_id:
                self._logger.info('Interface {0} has vlan, so cannot add to port-channel'.format(
    ...
    def delete_portchannel(self, port_list):
        ...
        for port in port_list.split(';'):
            port_name = port.split('/')[-1].replace('-', '/')
            port_channel = [re.search('channel.*\s(\d+)', line.strip()) for line in self._send_command(
                'show running-config interface ' + port_name + ' | include channel').splitlines() if
                            line.strip().startswith('channel')]
        ...
        # Take VLAN out before deleting the port-channel, otherwise the ports keep the vlan
        vlan_id = [line for line in self._send_command(
            'show running-config interface port-channel ' + port_channel_id + ' | include vlan').splitlines() if
    ...
    def restore_port_config(self, port_list):
        ...
        for port in port_list.split('|'):
            ...
            try:
                mtu = re.search('mtu=(.*?)', port_descr).group(1)
            except:
